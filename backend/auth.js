...
const transporter = nodemailer.createTransport({
  ...
  auth: {
    ...
    pass: process.env.GMAIL_PASS  // Gmail 應用程式密碼，從 .env 讀取，不是 Gmail 登入密碼
  }
})
...
router.post('/register', async (req, res) => {
  ...
  const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/ // 正規表達式：不含空白和@，接@，不含空白和@，接.，不含空白和@
  if (!emailRegex.test(email)) { // .test() 測試字串是否符合格式，不符合回傳 false
    return res.status(400).json({ error: 'Email格式不正確' }) // 400 客戶端錯誤：Email 格式錯誤
  }

  // 防呆三：ID 只能英文和數字，長度 4-20 字
  const idRegex = /^[a-zA-Z0-9]{4,20}$/ // 正規表達式：只允許英文大小寫和數字，長度 4 到 20
  if (!idRegex.test(custom_id)) { // 不符合格式就回傳錯誤
    return res.status(400).json({ error: 'ID只能使用英文和數字，長度4-20字' }) // 400 客戶端錯誤：ID 格式錯誤
  }

  // 防呆四：密碼至少 6 位數
  if (password.length < 6) { // .length 取字串長度
  ...
  verificationCodes[email] = {
    code,                                // 簡寫，等於 code: code
    expireAt: Date.now() + 5 * 60 * 1000 // 5 分鐘後過期
  }

  // 組合驗證連結，token 帶在 URL 參數裡，玩家點連結時伺服器用 token 查詢玩家
  const verifyLink = `${process.env.BACKEND_URL}/auth/verify-link?token=${token}`

  // TODO: 替換 - 等第三組設計好 Email 模板後，替換這裡的 html 內容
  await transporter.sendMail({
    from: `知識王 <${process.env.GMAIL_USER}>`, // 寄件人顯示名稱和地址，玩家收到信時看到的名字
    to: email,                                   // 收件人，就是剛剛註冊的玩家
    subject: '知識王 - 帳號驗證',                // 信件主旨
    html: `
  ...
  res.json({ message: '註冊成功，請查收驗證信' }) // 200 成功，回傳成功訊息
})
...
router.post('/verify', async (req, res) => {
  const { email, code } = req.body // 解構取出前端傳來的兩個欄位
  ...
  const record = verificationCodes[email] // 取出該 email 的驗證碼記錄，如果沒有就是 undefined
  ...
  // 用 != 而不是 !==，因為前端傳來的 code 是字串，暫存的是數字，用 != 可以自動轉型比對
  if (record.code != code) {
  ...
  const { error } = await supabase
    .from('users')
    .update({ is_verified: true, verify_token: null }) // 把 is_verified 改成 true，清空 token
...
router.get('/verify-link', async (req, res) => {
  ...
  const { data: userData, error } = await supabase
    .from('users')
    .select('id, email, is_verified') // 只需要這三個欄位
  ...
  await supabase
    .from('users')
    .update({ is_verified: true, verify_token: null }) // 開通帳號，清空 token 避免重複使用
    .eq('id', userData.id) // 條件：只更新這個玩家的資料

  // 確認 Supabase Auth 的 email，讓玩家可以登入
  await supabase.auth.admin.updateUserById(
    userData.id,            // 玩家的 id
    { email_confirm: true } // 把 email 標記為已確認
  )
...
router.post('/login', async (req, res) => {
  const { identifier, password } = req.body // 解構取出前端傳來的兩個欄位
  ...
  if (!identifier.includes('@')) {
    const { data, error } = await supabase
      ...
      .eq('custom_id', identifier) // 找 custom_id 等於這個 identifier 的玩家
      .single() // 只取一筆
  ...
  // 用 email 和密碼登入 Supabase Auth，密碼會自動比對加密後的版本
