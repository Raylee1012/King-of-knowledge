...
def update_stats():  # 處理對戰結束的統計更新：勝敗場、XP、金幣、等級、題目分類統計
    try:  # 包覆整個處理流程，捕捉任何未預期的伺服器錯誤並回傳 500
        ...
        is_draw = data.get('is_draw') is True  # 是否平手
        ...
        if won:  # 判斷本場是否勝利
            ...
            wins += 1  # 勝場數加 1
        elif is_draw:  # 平手：扣入場費但依答對題數給予金幣獎勵，勝敗場數不計
            if is_room_match:
                coin_delta = 0  # 房號配對不計金幣
            else:
                coin_delta = 0 if mode == 'bot' else -50 + 20 * correct  # 平手金幣：扣入場費 50，每答對 1 題回饋 20 金幣
            xp_gain = int(max(5, 3 * correct))  # 平手 XP：同失敗公式，至少 5 XP
        else:  # 失敗
            if is_room_match:
                coin_delta = 0  # 房號配對不計金幣
            else:
