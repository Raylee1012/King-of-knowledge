...
function updatePlayerBar() {
  ...
  [
    ['playerFrame', 'avatar-frame'],
    ['playerFrame3', 'avatar-frame']
  ].forEach(([id, baseClass]) => {
    const el = document.getElementById(id);
    if(el) el.className = baseClass + ' ' + (state.equippedFrame !== 'frame-none' ? state.equippedFrame : '');
  });
}
...
function getAvatarEditorMetrics() {
  ...
  return {
    stageSize,
    displayWidth: img.naturalWidth * baseScale * zoom,
    displayHeight: img.naturalHeight * baseScale * zoom
  };
}
